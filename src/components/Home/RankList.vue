<template>
    ...
    <div class="bg">
            <div class="header">
                <img width="100%" height="100%" :src="songsList.songListPic">
                <span @click="delmusic"> 退出 </span>
                <strong>歌单</strong>
            </div>
            <div class="header-text">
                <span>{{songsList.songListName}}</span>
                <p>{{songsList.songListPlayCount}}</p>
            </div>
        </div>
     ...
     <div class="songlists-List">
            <div class="songlists-all">
                <i >11</i>
                <span>播放全部</span>
                <span>（共{{songsList.songs.length}}首）</span>
            </div>
            <div class="songlists-itme">
                <ul>
                    <li v-for="(itme,index) in songsList.songs" :key="index"  @click="musicplays(itme,index)"  >
                        <div class="content">
                            <span>{{index + 1}}</span>
                            <h2>{{itme.name}}</h2>
                            <p>{{itme.singer}}</p>
...
<script>
   ...
   // const url="/api/music/netease/songList?key=579621905&id=2483853001&limit=10&offset=0"
    export default {
        name: "rankList",
        props:['isranking','isloading','RankListss','songsList'],
        data(){
            return{
                songlist:[],
                songlists:[],
